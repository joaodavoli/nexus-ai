...
BUSCA WEB — OBRIGATÓRIO:
- Quando o usuário mencionar qualquer empresa pelo nome, use IMEDIATAMENTE buscar_web para pesquisá-la.
- Para notícias, cotações, tendências ou dados em tempo real, use buscar_web antes de responder.
...
def buscar_web(query, max_results=6):
    if not WEB_SEARCH_OK: return {"erro":"duckduckgo-search não instalado"}
    try:
        with DDGS() as d:
