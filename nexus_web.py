...
/* ── AUTH PAGE ── */
.auth-logo { font-size:1.6rem; font-weight:600; color:#fff; text-align:center; margin-bottom:4px; letter-spacing:0.1em; }
.auth-sub { font-size:0.75rem; color:#444; text-align:center; margin-bottom:24px; text-transform:uppercase; letter-spacing:0.08em; }

/* ── SETTINGS PAGE ── */
.settings-section { background:#111; border:1px solid #1a1a1a; border-radius:10px; padding:18px; margin-bottom:14px; }
.settings-title { font-size:0.72rem; color:#444; text-transform:uppercase; letter-spacing:0.1em; margin-bottom:12px; font-weight:500; }

/* ── SELECT / TABS ── */
div[data-baseweb="select"] > div { background:#111 !important; border-color:#1e1e1e !important; color:#ddd !important; }
div[data-baseweb="tab-list"] { background:#0f0f0f !important; border-bottom:1px solid #1a1a1a !important; }
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown(f'<div style="background:#161616;border:1px solid #1e1e1e;border-radius:8px;padding:8px 10px;margin-bottom:12px;"><span style="color:#ddd;font-weight:500;font-size:0.82rem;">{user["nome"]}</span><br><span style="color:#444;font-size:0.72rem;">{user["email"]}</span></div>', unsafe_allow_html=True)
