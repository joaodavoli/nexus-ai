...
html, body, [class*="css"] {{ font-family: 'Inter', sans-serif; color: {text} !important; }}
...
# ─── Prompt ────────────────────────────────────────────────────────────────────
BASE_PROMPT = """Você é NEXUS — assistente executivo de IA para CEOs, fundadores e líderes empresariais.
...
Quando o usuário perguntar sobre notícias, empresas, mercados ou qualquer dado em tempo real, use SEMPRE a ferramenta buscar_web para obter informações atualizadas antes de responder.
...
def get_system_prompt():
    ...
    empresa = user.get("configuracoes", {}).get("empresa", "").strip() if user else ""
    if empresa:
        # Tenta extrair o nome da empresa (primeira linha ou primeiras palavras)
        nome_empresa = empresa.split("—")[0].split("-")[0].split(",")[0].split("\n")[0].strip()
        prompt += f"""

═══ EMPRESA DO USUÁRIO ═══
{empresa}

INSTRUÇÕES CRÍTICAS SOBRE A EMPRESA:
1. Esta é a empresa REAL do usuário. Use sempre como contexto principal.
2. Ao iniciar qualquer conversa ou quando o usuário mencionar a empresa, use OBRIGATORIAMENTE a ferramenta buscar_web para pesquisar: "{nome_empresa}" e "{nome_empresa} notícias 2024 2025"
3. Combine o que o usuário descreveu com o que você encontrar na web para dar respostas completas e atualizadas.
4. Se o usuário perguntar qualquer coisa relacionada à empresa, busque informações atuais antes de responder."""
    return prompt
...
def tela_configuracoes():
    ...
    st.markdown('<div class="settings-section">', unsafe_allow_html=True)
    st.markdown('<div class="settings-title">🏢 Minha Empresa (NEXUS usará isso em todas as respostas)</div>', unsafe_allow_html=True)
    empresa_info = st.text_area(
        "Descreva sua empresa",
        value=cfg.get("empresa",""),
        height=160,
        placeholder="Ex: Empresa Menrad — fundada em 2018, setor de construção civil, 45 funcionários, faturamento R$2M/ano, atua em SP e RJ, produto principal: estruturas metálicas para galpões industriais. Concorrentes: X, Y. Desafio atual: expandir para o Nordeste..."
    )
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    if st.button("Salvar configurações", use_container_width=True):
        atualizar_usuario(user["email"],{"nome":novo_nome,"configuracoes":{**cfg,"max_tokens":max_tokens,"idioma":idioma,"empresa":empresa_info}})
        users=carregar_usuarios()
        if user["email"] in users: st.session_state.user=users[user["email"]]
        st.session_state.user["nome"]=novo_nome
        st.session_state.user.setdefault("configuracoes",{})["empresa"]=empresa_info
        st.success("✅ Salvo! O NEXUS agora conhece sua empresa.")
    st.markdown('</div>', unsafe_allow_html=True)
...
def main():
    ...
    # Busca automática da empresa ao iniciar (uma vez por sessão)
    empresa_cfg = user.get("configuracoes",{}).get("empresa","").strip()
    if empresa_cfg and not st.session_state.get("empresa_buscada"):
        nome_emp = empresa_cfg.split("—")[0].split("-")[0].split(",")[0].split("\n")[0].strip()
        st.session_state["empresa_buscada"] = True
        prompt_auto = f"Pesquise agora na web sobre a empresa '{nome_emp}' e me dê um briefing rápido com as informações mais recentes encontradas: o que ela faz, notícias recentes, posição no mercado. Seja direto."
        with st.spinner(f"🔍 Buscando informações sobre {nome_emp}..."):
            r = processar_resposta(prompt_auto)
